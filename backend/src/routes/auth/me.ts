...
export default async function me(req : Request, res : Response) {
  try{
    ...
    if(!decoded || !decoded.userId) return res.status(401).send({ message: "Invalid token" , path : "login" });
    ...
    if(!user) return res.status(401).send({ message: "User not found" , path : "login" });
